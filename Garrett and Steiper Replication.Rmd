...
We can compare these values to this handy table of reported P-values from the paper: 
...
<br> Now, I think I will end the analytical section by trying to recreate some of the figures. 
...
I believe I can do something like this with ggPlot (not sure about the little color key graphic on the side though), so let's see how close we can get. 

<br>First I will try plot A. As you will notice, I will need to add a new column in order to scale the colors correctly, which I will do first. 
...
g <- ggplot(data = Olfaction, aes(x= log.Ethmoid.Area, y= Total.OR.Genes, color = logHBL)) + geom_point() + xlim(2, 9)
g
...
g2 <- g1 + geom_point(aes(size = prop.functional.OR.Genes))
g2
